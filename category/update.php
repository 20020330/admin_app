...
$result = $database->getReference("category")->getValue();
            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link  active" href="../category">
                        <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <svg style="fill: #fff" xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
                                <style>
                                </style>
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="javascript:;">Categories</a></li>
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Update Categories</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Update Categories</h6>
                </nav>
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    ...
                    <ul class="navbar-nav  justify-content-end">
                        <li class="nav-item d-flex align-items-center">
                            <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="../category">List Category</a>
                            <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="../category/insert.php">Add Category</a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="card px-5 py-3">
                        <form id="form_category" class="row justify-content md-center">
                            <div class="col-3 mt-3">
                                <div class="container bg-gradient-primary opacity-5 rounded d-flex align-items-center justify-content-center" style="width: 100%; height: 100%; display: none !important;" id="backdrop_thumbnail" name="backdrop_thumbnail">
                                    <p for="category_name " class="h4 d-block" style="color: #fff; text-align: center; ">Add Category Thumbnail</p>
                                </div>
                                <?php
                                $thumbnailValue = "";
                                foreach ($result as $key => $row) {
                                if ($row != null) {
                                    $thumbnailValue =  $row["thumbnail"];
                                }
                                echo "<img id='preview_thumbnail' src='$thumbnailValue' name='preview_thumbnail' class='img-fluid ratio ratio-1x1 rounded' style='object-fit: cover;' />";
                                echo "<script> const oldThumbnail = '$thumbnailValue'</script>";
                                ?>
                            ...
                            <div class="col-9">
                                <div class="form-group">
                                    ...
                                    echo "<input class='form-control' value='$nameValue' type='search' id='category_name' name='category_name'>"
                                    ?>
                                </div>
                                <div class="form-group">
                                    <label for="category_info" class="form-control-label">Category Info</label>
                                    <?php
                                    $infoValue = "";
                                    if ($row != null) {
                                        $infoValue =  $row["info"];
                                    }
                                    echo "<textarea class='form-control' id='category_info' name='category_info' rows='3'>" .  trim($infoValue) . "</textarea>";
                                    ?>
                                </div>
                                <div class="form-group">
                                    <label for="category_color" class="form-control-label">Category Color</label>
                                    <?php
                                    $colorValue = "";
                                    if ($row != null) {
                                        $colorValue =  $row["display_color"];
                                    }
                                    echo "<input class='form-control' value='$colorValue' id='category_color' name='category_color' type='color' />";
                                }
                                    ?>
                                </div>
                                <div class="form-group gap-3">
                                    <button type="button" class="btn bg-gradient-primary mt-3 me-3" id="btn_submit" name="btn_submit">Update Category</button>
                                    <input type="reset" class="btn bg-gradient-default mt-3 form-control-lg" name="btn_clear" value="Clear Form" />
                                </div>
                            </div>
                        </form>
                    </div>
...
<script type="module">
    ...
    buttonSubmitCategory.addEventListener("click", (e) => {
        const categoryName = form.category_name.value.trim()
        const categoryInfo = form.category_info.value.trim()
        const categoryColor = form.category_color.value.trim()

        if (categoryColor.length > 0 && categoryInfo.length > 0 && categoryName.length > 0) {
            if (hasNewThumbnail) {
                if (fileInput.files && fileInput.files[0]) {
                    sendImageToFirebase("categories", fileInput.files[0], (urlImage, status) => {
                        const categoryThumbnail = urlImage?.trim()
                        if (categoryThumbnail.length === 0) {
                    ...
                    appAlert("Lỗi thumbnail của thể loại!!", "Hmmmm!", "warning", true)
                }
            } else {
                if (oldThumbnail) {
                    handleSubmitCategory({
                        ...
                        category_info: categoryInfo,
                        category_name: categoryName,
                        category_thumbnail: oldThumbnail?.trim()
                    })
                } else {
                    appAlert("Lỗi thumbnail của thể loại!!", "Hmmmm!", "warning", true)
                }
    ...
    function handleSubmitCategory(data) {
        fetchData("POST", "handlePost.php", {
            ...
            if (data.status === "success") {
                appAlert("Cập nhật loại thành công!", "Huray!", "success", true)
