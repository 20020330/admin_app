...
// $result = $dbConn->query("SELECT id, title, id_comic, chapter_index FROM chapter");
$comicResult = $dbConn->query("SELECT id, title FROM comic");
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Chapters</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Chapters</h6>
                </nav>
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    <div class="ms-md-auto pe-md-2 d-flex align-items-center">
                        ...
                        <select id="select_comic" class="form-control" aria-label="Default select example">
                            <option value="" selected>Chọn truyện</option>
                            <?php
                            while ($rowComic = $comicResult->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Chapter Index</th>
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Id Comic</th>
...
<script>
    const selectComic = document.querySelector("#select_comic");
    ...
    function handleDeleteItem(btn_delete_comic) {
        appAlertConfirm(() => {
            const id = btn_delete_comic.getAttribute('data-id').trim();
            // console.log(id);
            fetchData("POST", `chapter/handlePost.php`, {
                type_handle: "delete",
                chapter_id: id
            }).then(data => {
                if (data.status === "success") {
                    appAlert("Xóa chương thành công!", "Huray!", "success")
                    setTimeout(() => {
                        fetchDataChapter()
                        Swal.close();
                    }, 500)
    ...
    function fetchDataChapter() {
        if (selectComic.value.trim() != "") {
            ...
            fetch(`chapter/handleGet.php?id_comic=${selectComic.value.trim()}`, {
                    headers: {
                        "Content-Type": "application/json",
                    },
                })
                ...
                <tr>
                        ...
                        <td>
                            <p class='text-xs font-weight-bold mb-0'>${item.chapter_index}</p>
                        </td>
                        <td>
                            <p class='text-xs font-weight-bold mb-0'>${item.id_comic}</p>
                        </td>
                        <td class='align-middle'>
                            <a href="chapter/update.php?chapter_id=${item.id}" class='badge bg-gradient-info' role='button' aria-pressed='true' data-bs-toggle='tooltip' data-bs-placement='top' title='Detail Chapter' data-container='body' data-animation='true'>
                            ...
                            <a onclick='handleDeleteItem(this)' data-id='${item.id}' class='badge bg-gradient-danger' role='button' aria-pressed='true' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete Chapter' data-container='body' data-animation='true'>
    ...
    function appAlertConfirm(whenAcceptCallback) {
        Swal.fire({
            title: 'Xác nhận',
            ...
            customClass: {
                confirmButton: "btn bg-gradient-info mx-3",
            ...
            if (result.isConfirmed) {
                ...
                appAlert("Chương vẫn còn nguyên <3", "Hmmmm")
            }
        })
    }
</script>