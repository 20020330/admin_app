...
$result = $dbConn->query("SELECT name, info FROM author WHERE id = '$author_id'");
    ...
    <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
        ...
        <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link  active" href="../author">
                        <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm">
                            <a class="opacity-5 text-dark" href="javascript:;">Authors</a>
                        </li>
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Update Author</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Update Author</h6>
                </nav>
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    ...
                    <ul class="navbar-nav  justify-content-end">
                        <li class="nav-item d-flex align-items-center">
                            <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="../author">Author List</a>
                        </li>
                        <li class="nav-item d-flex align-items-center">
                            <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="../author/insert.php">Add Author</a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="card px-5 py-3">
                        <form id="form_author">
                            <div class="form-group">
                                ...
                                echo "<input class='form-control' value='$nameValue' type='search' id='author_name' name='author_name'>"
                                ?>
                            </div>
                            <div class="form-group">
                                <label for="author_info" class="form-control-label">Author Info</label>
                                <?php
                                $infoValue = "";
                                if ($row != null) {
                                    $infoValue =  $row["info"];
                                }
                                echo "<textarea class='form-control' rows='3' id='author_info' name='author_info'>" .  trim($infoValue) . "</textarea>";
                                ?>
                            </div>
                            <div class="form-group gap-3">
                                <button type="button" class="btn bg-gradient-primary mt-3 me-3" id="btn_submit" name="btn_submit">Update Author</button>
...
<script>
    const form = document.querySelector("#form_author")
    const buttonSubmitAuthor = document.querySelector("#btn_submit")

    buttonSubmitAuthor.addEventListener("click", (e) => {

        const authorName = form.author_name.value.trim()
        const authorInfo = form.author_info.value.trim()

        if (authorInfo.length === 0 || authorName.length === 0) {
            appAlert("Không để trống tên hay thông tin của tác giả!!", "Hmmmm!", "warning")
        } else {
            handleSubmitAuthor(authorName, authorInfo)
        }
    })

    function handleSubmitAuthor(authorName, authorInfo) {
        postData("handlePost.php", {
            author_id: new URLSearchParams(window.location.search).get("author_id"),
            author_name: authorName,
            author_info: authorInfo,
            type_handle: "update"
        }).then(data => {
            if (data.status === "success") {
                appAlert("Cập nhật tác giả thành công tác giả thành công!", "Huray!", "success")
                setTimeout(500, () => {
