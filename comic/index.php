...
$result = $dbConn->query("SELECT cm.title, cm.sub_title, cm.thumbnail, cm.id, cm.country, au.name AS author, us.user_name AS publish_by FROM `comic` AS cm 
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Comics</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Comics</h6>
                </nav>
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Publish By</th>
                            ...
                            <tbody>
                                ...
                                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    echo "<td>
                                            ...
                                            <p class='text-xs text-secondary mb-0'>" . $row['sub_title'] . "</p>
                                          </td>";
                                    echo "<td>
                                            <img id='preview_thumbnail' src='" . $row['thumbnail'] . "' class='img-thumbnail' width='120' />
                                          </td>";
                                    echo "<td>
                                            <p class='text-xs font-weight-bold mb-0'>" . $row['author'] . "</p>
                                          </td>";
                                    echo "<td>
                                            <p class='text-xs font-weight-bold mb-0'>" . $row['country'] . "</p>
                                          </td>";
                                    echo "<td>
                                            <p class='text-xs font-weight-bold mb-0'>" . $row['publish_by'] . "</p>
                                          </td>";
                                    echo "<td class='align-middle'>
                                                <a href='comic/update.php?comic_id=" . $row['id'] . "'class='badge bg-gradient-info' role='button' aria-pressed='true' data-bs-toggle='tooltip' data-bs-placement='top' title='Detail Comic' data-container='body' data-animation='true'>
                                                ...
                                                <a onclick='handleDeleteItem(this)' data-id='" . $row["id"] . "' class='badge bg-gradient-danger' role='button' aria-pressed='true' data-bs-toggle='tooltip' data-bs-placement='bottom' title='Delete Comic' data-container='body' data-animation='true'>
                                                    <svg style='fill: #fff;' xmlns='http://www.w3.org/2000/svg' height='1em' viewBox='0 0 448 512'><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
...
<script>
    function handleDeleteItem(btn_delete_comic) {
        appAlertConfirm(() => {
            ...
            // console.log(id);
            fetchData("POST", `comic/handlePost.php`, {
                type_handle: "delete",
                comic_id: id
            }).then(data => {
                if (data.status === "success") {
                    appAlert("Xóa truyện thành công!", "Huray!", "success")
                    setTimeout(() => {
                        location.reload()
                    }, 500)
                } else {
    ...
    function appAlertConfirm(whenAcceptCallback) {
        Swal.fire({
            title: 'Xác nhận',
            text: "Bạn có chắc chắn muốn xóa truyện này?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                appAlert("Truyện vẫn còn nguyên <3", "Hmmmm")
            }
        })
    }
</script>