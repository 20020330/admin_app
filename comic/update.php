...
$resultComic = $dbConn->query(
    "SELECT cm.title, cm.sub_title, cm.thumbnail, cm.id, cm.synopsis, cm.country, au.id AS author, us.user_name AS publish_by FROM `comic` AS cm 
                            INNER JOIN `author` AS au ON cm.author = au.id
                            INNER JOIN `user` AS us ON cm.publish_by = us.id
                            WHERE cm.id = '$comicId'"
)->fetch(PDO::FETCH_ASSOC);

$resultComicCategory = $dbConn->query("SELECT cate.id, cate.title FROM comic_category AS cm_cate
                                        INNER JOIN category AS cate ON cm_cate.id_category = cate.id
                                        WHERE cm_cate.id_comic = '$comicId'");

// echo var_dump($resultComicCategory->fetch(PDO::FETCH_OBJ));
?>
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Update Comic</li>
                    </ol>
                    <h6 class="font-weight-bolder mb-0">Update Comics</h6>
                </nav>
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                    ...
                    <ul class="navbar-nav  justify-content-end">
                        ...
                        <li id="btn_add_comic" class="nav-item d-flex align-items-center">
                            <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="../comic/insert.php">Add Comic</a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="card px-5 py-3">
                        <form id="form_category" class="row justify-content md-center">
                            <div class="col-3 mt-3">
                                <div class="container ratio ratio-1x1 bg-gradient-primary opacity-5 rounded d-flex align-items-center justify-content-center" style="width: 100%; height: 100%; display: none !important;" id="backdrop_thumbnail" name="backdrop_thumbnail">
                                    <p for="category_name " class="h4 d-block" style="color: #fff; text-align: center;">Add Comic Thumbnail</p>
                                </div>
                                <?php
                                echo "<img id='preview_thumbnail' src='" . $resultComic['thumbnail'] . "' name='preview_thumbnail' class='ratio ratio-1x1 rounded img-fluid' style='object-fit: cover; height: 100% !important' />";
                                echo "<script> const oldThumbnailUrl = '" . $resultComic['thumbnail'] . "'  </script>";
                                ?>
                            ...
                            <div class="col-9">
                                <div class="form-group">
                                    ...
                                    <?php
                                    echo "<input class='form-control' value='" . $resultComic['title'] . "' type='search' id='comic_title' name='comic_title'>"
                                    ?>
                                </div>
                                <div class="form-group">
                                    <label for="comic_sub_title" class="form-control-label">Comic Sub Title</label>
                                    <?php
                                    echo "<input class='form-control'value='" . $resultComic['sub_title'] . "' type='search' id='comic_sub_title' name='comic_sub_title'>"
                                    ?>
                                </div>
                                <div class="form-group">
                                    <label for="comic_synopsis" class="form-control-label">Comic Synopsis</label>
                                    <textarea class="form-control" id="comic_synopsis" name="comic_synopsis" rows="3"><?php echo trim($resultComic['synopsis']); ?></textarea>
                                </div>
                                <div class="row">
                                    <div class="col">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control " id="comic_country" name="comic_country">
                                                ...
                                                while ($rowCountries = $resultCountries->fetch(PDO::FETCH_ASSOC)) {
                                                    if ($rowCountries["name"] == $resultComic["country"]) {
                                                        echo "<option selected='selected' value='" . $rowCountries["name"] . "'>" . $rowCountries["name"] . "</option>";
                                                    } else {
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="comic_author" name="comic_author">
                                                ...
                                                while ($rowAuthor = $resultAuthor->fetch(PDO::FETCH_ASSOC)) {
                                                    if ($rowAuthor["id"] == $resultComic["author"]) {
                                                        echo "<option selected='selected' value='" . $rowAuthor["id"] . "'>" . $rowAuthor["name"] . "</option>";
                                                    } else {
                                ...
                                <div class="form-group">
                                    ...
                                    <?php
                                    $listCategoryInit = array();

                                    while ($row = $resultComicCategory->fetch(PDO::FETCH_ASSOC)) {
                                        $listCategoryInit[] = $row["id"];
                                    }

                                    echo "<select class='form-control' id='comic_category' name='comic_category' multiple>";
                                    ...
                                    echo "<script>
                                        ...
                                        $(document).ready(function() {
                                            ...
                                            $('#comic_category').val(" . json_encode($listCategoryInit) . ").trigger('change');
                                        });
                                    </script>"
                                    ?>
                                </div>
                                <div class="form-group gap-3">
                                    <button type="button" class="btn bg-gradient-primary mt-3 me-3" id="btn_submit" name="btn_submit">Update Comic</button>
...
<script type="module">
    ...
    $("#btn_submit").click((e) => {
        ...
        const isOk = hasComicSubTitle && hasComicSynopsis && hasComicTitle && hasAuthor && hasCountry && hasCategories

        if (isOk) {
            if (!hasNewThumbnail) {
                handleSubmitCategory({
                    comic_id: new URLSearchParams(window.location.search).get("comic_id"),
                    comic_title: comicTitle,
                    comic_sub_title: comicSubTitle,
                    comic_synopsis: comicSynopsis,
                    comic_thumbnail: oldThumbnailUrl?.trim(),
                ...
                if (hasImageThumbnail) {
                    ...
                    appAlert("Ảnh của truyện bị lỗi. Vui lòng chọn ảnh mới!!", "Hmmmm!", "warning", true)
                }
            }
        } else {
            appAlert("Không để trống thông tin nào của truyện!!", "Hmmmm!", "warning", true)
        }
    })
    ...
    // function initValueCategories(arrayValue) {
    ...
    async function sendImageToFirebase(refName, image, callback) {
        ...
        appAlert("Đang cập nhật dữ liêu:<<", "Hmmmmm!!", "", {
            didOpen: () => {
                Swal.showLoading()
            },
        }, false)
    ...
    function handleSubmitCategory(data) {
        fetchData("POST", "handlePost.php", {
            ...
            if (data.status === "success") {
                appAlert("Cập nhật tuyện thành công!", "Huray!", "success", true)
